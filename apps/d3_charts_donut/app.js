lbs.apploader.register('d3_charts_donut', function () {
    ...
    this.initialize = function (viewModel, element) {
        ...
        this.setD3Helpers();
        this.createVisualObjects(element);
        this.update();

        return viewModel;
    }

    this.setD3Helpers = function(){
    ...
    this.createVisualObjects = function (element) {
        self.vis = d3.select($(element).find(".chart_donut").get(0)).append("svg:svg")
          .attr("width", w)
          .attr("height", h);

        //GROUP FOR ARCS/PATHS
        self.arc_group = self.vis.append("svg:g")
          .attr("class", "arc")
        ...
        self.paths = self.arc_group.append("svg:circle")
            .attr("fill", "#EFEFEF")
            .attr("r", r);


        //WHITE CIRCLE BEHIND LABELS
        self.whiteCircle = self.center_group.append("svg:circle")
          .attr("fill", "white")
          .attr("r", ir);

        // "TOTAL" LABEL
        self.totalLabel = self.center_group.append("svg:text")
          .attr("class", "label")
          .attr("dy", -15)
        ...
        //TOTAL TRAFFIC VALUE
        self.totalValue = self.center_group.append("svg:text")
          .attr("class", "total")
          .attr("dy", 7)
          .attr("text-anchor", "middle") // text-align: right
          .text("Waiting...");
        ...
        //UNITS LABEL
        self.totalUnits = self.center_group.append("svg:text")
          .attr("class", "units")
          .attr("dy", 21)
          .attr("text-anchor", "middle") // text-align: right
          .text("kb");
    ...
    this.update = function() {
        ...
        self.arraySize = Math.ceil(Math.random() * 10);
        self.streakerDataAdded = d3.range(arraySize).map(self.fillArray);

        self.oldPieData = self.filteredPieData;
        self.pieData = self.donut(streakerDataAdded);
